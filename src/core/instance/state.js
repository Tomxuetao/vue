...
/**
 * 代理 通过 sharedPropertyDefinition 对象 给key添加一层getter和setter  将key代理到 vue 实例上
 * @param target
 * @param sourceKey
 * @param key
 */
...
/**
 * 初始化数据,响应式原理的入口(初始化顺序：props --> methods --> data --> computed --> watch)
 * @param vm
 */
export function initState (vm: Component) {
  // 为当前组件实例创建了一个watchers属性，vm._watchers保存着当前组件实例的所有监听者（watcher）
  vm._watchers = []
  // 从实例上获取配置项
...
function initData (vm: Component) {
  ...
  /**
   * 循环判断data中的属性和props,methods中的属性是否冲突，因为所有的data,props,methods最终都会挂载到vm实例上
   */
  let i = keys.length
  while (i--) {
    const key = keys[i]
...
/**
 * 为 sharedPropertyDefinition 添加 get， set 属性,将该 computed 属性添加到 Vue 实例 vm 上，并使用 sharedPropertyDefinition 作为设置项。
 * @param target
 * @param key
 * @param userDef
 */
...
/**
 * 在这里我们暂时只需要知道sharedPropertyDefinition的 get属性 被设置为这个方法的返回值就行
 * @param key
 * @returns {(function(): (Watcher|undefined))|*}
 */
...
/**
 * 兼容性处理，保证 handler 肯定是一个函数, 调用 $watch
 * @param vm
 * @param expOrFn
 * @param handler
 * @param options
